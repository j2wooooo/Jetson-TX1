...
void read_trainData(string fn, Mat& trainingData, Mat& labels) { // 파일로 저장된 학습데이터를 읽어서 행렬(trainingData, labels)에 저장
...
int main() {
	...
	trainingData = trainingData.reshape(1,trainingData.rows);  // 대박 -> svm 하려면 데이터가 1*n 형태여야 함
